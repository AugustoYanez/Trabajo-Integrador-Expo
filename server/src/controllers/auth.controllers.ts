...
export const login = async (req: IReq, res: IRes) => {
    ...
    if (!found) return res.status(400).json(["usuario no encontrado"]); 

    const match = await bcrypt.compare(contraseña, found.contraseña)
    if(!match) return res.status(500).json(["password incorrect"]);
...
export const register = async (req: IReq, res: IRes) => {
    ...
    if (found) return res.status(400).json(["nombre/email en uso"]);
